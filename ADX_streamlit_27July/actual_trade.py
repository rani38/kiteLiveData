...
def setOffset(offset):
    ...
    timeframe = []
    conn, cur = connection()
    query = """SELECT * FROM adxdis ORDER BY datetime DESC LIMIT 3;"""
    cur.execute(query)
    rows = cur.fetchall()
    cur.close()
    conn.close()
    ...
    while True:
        ...
        try :
            if current_time >= starttime and current_time <= endtime:
                # Entry conditions
                if adx[2] > 20 and adx[2]<35 and adx[2] < adx[1] and adx[1] < adx[0]:
                    if plusDI[0] > minusDI[0] and not LE:
                        ...
                        # fucntion to take trade
                        trade_instance.buyShare()
